...
### Example 3: API Endpoint Inspection
...
depth: shallow
lens: ["security"]
context: "REST API health check"
```

**Output:**
```yaml
inspection:
  target_ref: "https://api.example.com/v2/users"
  resolved_to: "https://api.example.com/v2/users"
  inspected_at: "2024-01-15T12:00:00Z"
identity:
  id: "/v2/users"
  type: "REST endpoint"
  name: "Users endpoint"
  namespace: "api.example.com/v2"
  description: "User management API endpoint"
structure:
  kind: flat
  shape: "RESTful resource endpoint"
  key_parts:
    - name: "GET /v2/users"
      type: "operation"
      location: null
      significance: "List users"
    - name: "POST /v2/users"
      type: "operation"
      location: null
      significance: "Create user"
  depth: null
  size_metrics:
    total_elements: null
    total_size: null
...
condition:
  status: healthy
  health_signals:
    - signal: "Responds to requests"
      polarity: positive
      evidence: "200 OK response in 145ms"
    - signal: "HTTPS enabled"
      polarity: positive
      evidence: "TLS 1.3 connection"
  risks:
    - risk: "Rate limiting headers missing"
      severity: low
      evidence: "No X-RateLimit-* headers in response"
  anomalies: []
lens_findings:
  security:
    tls_version: "1.3"
...
confidence: 0.8
evidence_anchors:
  - "tool:WebFetch:response_headers"
  - "tool:WebFetch:options_response"
assumptions:
  - "Endpoint is publicly documented"
  - "OPTIONS method accurately reflects capabilities"
```
