---
name: delegate
description: Split work across subagents with explicit contracts and merge plan.
argument-hint: "[goal] [subtasks] [contracts]"
...
## Intent
Execute **delegate** as a reusable, evidence-first capability.

Success means you return the output contract **with grounded evidence** and **explicit assumptions**.

## Inputs
- Required: the data/context needed to run `delegate`
...
## Output contract
Return:
- Subtasks + owners: <agent contracts>
- Interfaces: <inputs/outputs per task>
- Merge plan: <how to combine results>
